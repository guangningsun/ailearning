...
const GITHUB_BASE = 'https://github.com/guangningsun/ai-engineering-from-scratch/tree/main/';
const SITE_ORIGIN = 'https://aiengineeringfromscratch.com';

// Per-phase display names, keyed by phase id. When a phase id has an entry
// here, build.js uses it as the phase `name` in data.js, overriding whatever
// was parsed from README.md. This is the single source of truth for the
// displayed phase title on the 中文 site; add or change entries here.
const PHASE_NAMES_ZH = {
  0:  '环境搭建',
  1:  '数学基础',
  2:  '机器学习基础',
  3:  '深度学习核心',
...
function getPhaseName(id, parsedName) {
  return PHASE_NAMES_ZH[id] != null ? PHASE_NAMES_ZH[id] : parsedName;
}
...
function parseReadme(content, roadmapStatuses) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (phaseHeaderMatch) {
      ...
      currentPhase = { id, name: getPhaseName(id, name.trim()), status: phaseStatus, desc, lessons: [] };
    ...
    if (detailsHeaderMatch) {
      ...
      const roadmapKey = `Phase ${id}`;
      const phaseStatus = roadmapStatuses[roadmapKey]?.phaseStatus || 'planned';
      currentPhase = { id, name: getPhaseName(id, name.trim()), status: phaseStatus, desc: desc?.trim() || '', lessons: [] };
      phases.push(currentPhase);
      inLessonTable = false;
      continue;
    }
...
function writeLlms(phases, glossaryCount, artifactCount) {
  ...
  out += `Source: https://github.com/guangningsun/ai-engineering-from-scratch\n`;
